...
$VRAY_S3_URI = ""  # e.g. s3://<bucket>/vray_adv_70020_max2027_x64.exe
...
foreach ($name in "3DS_MAX_INSTALLER_ZIP_S3_URI","VRAY_S3_URI") {
...
if ($PERSISTENCE_ENABLED -and (Test-Path $INSTALL_MARKER)) {
    ...
    if (Test-Path "$MAX_ROOT\3dsmaxbatch.exe") {
        # The registry keys, services and env vars are on the OS disk, so put them back
